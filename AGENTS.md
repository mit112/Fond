...
This app targets **iOS 26, iPadOS 26, and watchOS 26** exclusively — no backward compatibility with iOS 18 or earlier. Always check current Apple documentation (WWDC25+) before writing SwiftUI, WidgetKit, or framework code. iOS 26 introduced Liquid Glass, new WidgetKit APIs, and significant SwiftUI changes. **Do not rely on pre-iOS 26 patterns or deprecated APIs** — when in doubt, look up the docs.
...
2. **Fond app** (views, app delegate, ContentView router)
...
- **Fond** — iOS / iPadOS app (iPhone + iPad)
...
`ContentView`/`AuthenticatedContentView` routes with local `@State` flags, not a single state-machine enum: `authManager.isSignedIn` → `isCheckingConnection` → `displayNameComplete` → `needsKeySync` → `isConnected`. View sequence: loading (Firebase init) → `SignInView` → loading (checking existing connection) → `DisplayNameView` → `KeySyncView` (key-sync edge case only) → `PairingView` → `ConnectedView`. Transitions animate with `.fondSpring`.

`ConnectionState` (`Shared/Models/ConnectionState.swift`: `signedOut`/`unpaired`/`waitingForPartner`/`syncingKeys`/`connected`) does **not** drive this routing — it's a snapshot written to the App Group UserDefaults bus (by `FirebaseManager`, `PushManager`, `SettingsView`, the NSE) that widgets, the NSE, and `FondRelevanceUpdater` read back to decide their own rendering/relevance, independent of the main app's navigation.
...
- `FondTheme` — `fondKeepsakeCard()` is an **opaque** card face (fill + amber `strokeBorder` + inset hairline + shadow) used for both the Now and Together card faces; there is no glass, blur, or gradient on content (`.fondCard()`/`GlassEffect.clear` no longer exist). Liquid Glass (`.glassEffect()`) survives only on floating controls — `fondFloatingControl(in:tinted:)` (toolbar) and `fondSendControl()` (tinted send button), each with an opaque Reduce-Transparency fallback — plus `fondControlPlate(in:)`, an opaque (non-glass) text substrate nested inside those glass containers. The old `.fondGlass()`/`.fondGlassInteractive()`/`.fondGlassPlain()` modifiers are gone.
- `FondOnboardingBackground` keeps the animated `MeshGradient` (3x3 grid, 6s breathing cycle; static `LinearGradient` on watchOS), but only for the pre-connection onboarding/loading screens — the connected experience's field is the flat `FondField`/`FondColors.field`.
- `CardTurn.swift` (`FondFace`, `CardTurnContainer`) — the signature gesture: a 3D Y-axis flip between the "Now" and "Together" card faces, spring-interruptible, with a Reduce Motion cross-fade fallback.
- `FondTypography` (`FondType`) — Fraunces variable font for editorial display type (`partnerName`, `question`, `momentQuestion`); Newsreader variable font for shared human-authored words (`pullQuote`, `voice`); system SF Pro for controls/facts (`body`, `control`, `metadata`, `eyebrow`).
- Centralized haptics (`FondHaptics`), animation presets (`.fondSpring`, `.fondQuick`).
- `FondColors` — adaptive color factory with `adaptive(light:dark:)`, resolves per platform trait/appearance; watchOS always uses dark variant. Amber is the sole brand accent — no lavender/rose secondary palette; those hues survive only as small status-dot accents.
