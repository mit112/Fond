...
>
> **✅ UPDATE 2026-08-26 — verification-hardening is MERGED, and the `<1ms` decrypt claim is now measured.** The branch below is on `main` (merge commit `1fced63`, PR #1 MERGED, pushed). Both gates re-run green **on the merged tree**: Swift **45 passed / 0 failed** (39 Swift Testing in 13 suites + 9 XCTest UI); `functions` **51 tests / 5 suites**, exit 0. All four targets rebuild **0 errors, 0 compiler warnings, 93s**, every product emitted, and the NSE still embeds zero frameworks. Separately, `Fond/FondTests/DecryptPerformanceTests.swift` (`5cd8d0f`) **measured the NSE payload decrypt for the first time: 5.5µs median / 6.3µs p99** for the whole five-field payload on an **iPhone 15 (A16, iOS 27)**, 5,000 iterations under `ContinuousClock` — ~180× under the long-asserted "<1ms", and the test now *asserts* that budget so it is a regression guard. The contradictory `<1ms` / `<10ms` comments in `NotificationService.swift` are reconciled (`555d75b`). **This closes §D of the verification-hardening handoff.** Still pending, unchanged: §C ratification, the full attended P3 device matrix (the NSE, widget push, HealthKit and background push have still never run on hardware), and P4–P6.
>
> **✅ UPDATE 2026-07-18 (verification-hardening branch `worktree-verification-hardening` — not yet merged):** The loop-implementable **P0-backend + P1 + P2** work is **done and verified on simulators/emulator**: `functions/` builds + lints clean; real automated coverage now exists for the E2E-crypto core, Cloud Functions, and Firestore rules (**`FondTests`: 37 Swift Testing + 8 XCTest UI, 0 failures**; **`functions`: 51 tests / 5 suites** on the offline emulator); the dead `lookupPairingCode`/`isAuthorized` were removed (`FondUser` intentionally kept); documentation drift was corrected; and countdown cross-device sync was fixed (self-doc listener). This **supersedes §0's "automated tests are essentially zero" and §6's "verification 0%"** for the loop-testable layer — **attended real-device / App-Store verification (P3–P6) is still pending**. No crypto / schema / function-behavior / push-contract / platform-scope / deployment-floor changes.
...
- ⚠️ **Dead code (2 artifacts) — UPDATE 2026-07-18 (verification-hardening): RESOLVED (2 removed, `FondUser` retained by decision):** `FirebaseManager.lookupPairingCode(_:)` (zero call sites, superseded by server-side `linkUsers`) and the dead `isAuthorized` assignment in `HeartbeatManager.swift:71-77` (zero reads) were **removed** — confirmed dead before deletion, build+test green after. `FondUser` struct is still **never referenced** in Swift (`FirebaseManager` reads Firestore via raw dictionary access — the 13-field model is schema-documentation only), but was evaluated for deletion and **intentionally kept**: `project.pbxproj` wires the file into the `watchkitapp Watch App` and `widgetsExtension` targets via `PBXFileSystemSynchronizedBuildFileExceptionSet` entries, so removing it breaks those builds without a pbxproj edit — deliberately deferred, not dead-by-neglect.

### ⚠️ Tests — UPDATE 2026-07-18 (verification-hardening): unit/emulator coverage now exists [VERIFIED]
**Original finding (2026-07-17):** Effectively zero real tests — six empty `@Test func example()` placeholders, no assertions; `functions/` had no test files or `test` script.

**Current state:** `Fond/FondTests/` now has **37 Swift Testing tests in 12 suites** + **8 XCTest UI tests**, 0 failures. Suites added this pass: `CryptoPrimitiveTests` (AES-GCM + X25519/HKDF), `KeychainManagerTests`, `EncryptionManagerTests`, `KeyExchangeManagerTests`, `StatusAndPromptTests` (UserStatus degradation + DailyPrompt determinism + `FondMessage` codec), and `CountdownSyncTests` — alongside the pre-existing Ember Folio suites (`CardTurnMathTests`, `FondPaletteTests`, `RelationshipDateSummaryTests`, `TogetherMomentBuilderTests`/`TogetherThreadStoreTests`, `FondTests`) and the XCTest UI bundle (`FondUITests`, `EmberFolioUITests`, plus launch tests). `functions/` now has a `test` script and **5 Jest suites / 51 tests** — `linkUsers`, `notifyPartner`, `unlinkConnection`, `expireCodes`, plus Firestore security rules (`rules.test.ts`) — run against the offline emulator.

**Not covered by this:** real hardware or a live Firebase project. The NSE decrypt path, widget push, HealthKit, and an actual `firebase deploy` remain untested — attended, device/integration-level work (see §6 risk #2).
...
**Verified this session (2026-07-17):** git shape & stall timing · NSE + widget targets compile clean on the current toolchain (0 errors) · all schemes blocked by missing watchOS 26 runtime · zero real tests · no TODO debt · 2 dead-code artifacts · dependency pins · no privacy manifest · no export-compliance key · `aps-environment=development` · docs conflicts. **UPDATE 2026-07-18 (verification-hardening):** "zero real tests" and "2 dead-code artifacts" no longer hold as of this branch — see the updated Tests and Dead-code entries in §3 above (37 Swift + 8 XCTest tests, 51 functions tests; `lookupPairingCode`/`isAuthorized` removed, `FondUser` intentionally kept).
